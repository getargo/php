...
class Responder
{
    ...
    public function __invoke(Action $action, Payload $payload = null) : Response
    {
        $this->action = $action;
        ...
        switch ($status) {
            ...
            case Status::ERROR:
                ...
                $this->renderIntoResponse($response, $payload, 'status:Error');
                break;

            case Status::INVALID:
                $response->setCode(422);
                $this->renderIntoResponse($response, $payload, 'status:Invalid');
                break;
    ...
    ) : void
    {
        ...
        $path = dirname(__DIR__, 3) . '/resources/response/html';
